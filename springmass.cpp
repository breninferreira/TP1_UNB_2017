...
void Mass::step(double dt)
{
   ...
   */
   ...
   double aceleration = mass / force.y;
   double xPos = getPosition().x + getVelocity().x * dt ;
   double yPos = getPosition().y + getVelocity().y * dt - 0.5 * aceleration * dt * dt ;

  if (xmin + radius <= xPos && xPos <= xmax - radius) {
    position.x = xPos ;
  } else {
